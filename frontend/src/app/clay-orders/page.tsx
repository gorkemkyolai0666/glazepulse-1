...
import { formatClayOrderStatus, formatCurrency } from '@/lib/utils';

interface ClayOrderItem {
  id: string;
  customerName: string;
  clayBody: string;
  supplierName?: string;
...
export default function ClayOrdersPage() {
  const { token } = useAuth();
  const [items, setItems] = useState<ClayOrderItem[]>([]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.clayOrders.create(token, {
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="font-display text-3xl text-primary">Kil Siparişleri</h1>
        ...
        {showForm && (
          <Card className="kiln-card">
            <CardHeader>
              <CardTitle className="font-display">Kil Siparişi Ekle</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2 sm:col-span-2">
                  <Label htmlFor="clayBody">Kil Kategorisi</Label>
                  <Input id="clayBody" value={form.clayBody} onChange={(e) => update('clayBody', e.target.value)} required placeholder="Örn: İpek Saten Set" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="status" value={form.status} onChange={(e) => update('status', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatClayOrderStatus(s)}</option>
                    ))}
                  </select>
                </div>
        ...
        {!loading && items.length > 0 && (
          <div className="grid gap-3 sm:grid-cols-2">
            {items.map((item) => (
              <Card key={item.id} className="kiln-card">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.clayBody} · {item.supplierName || 'Tedarikçi belirtilmemiş'} · {formatCurrency(item.price)}
                      </p>
                    </div>
                  </div>
                  <Badge variant="secondary">{formatClayOrderStatus(item.status)}</Badge>
                </CardContent>
              </Card>
            ))}
          </div>
