...
import {
  formatDateTime,
  formatFinishingStatus,
  formatFinishingCategory,
} from '@/lib/utils';

interface FinishingChecklistItem {
  id: string;
  title: string;
  description?: string;
  category: string;
...
interface ListResponse {
  data: FinishingChecklistItem[];
  total: number;
}

const CATEGORIES = ['edge_gilding', 'marbling', 'tooling', 'cover_design', 'quality_check', 'other'];
const STATUSES = ['scheduled', 'in_progress', 'completed', 'overdue'];

const emptyForm = {
  title: '',
  description: '',
  category: 'edge_gilding',
  zone: '',
  scheduledAt: new Date().toISOString().slice(0, 16),
  status: 'scheduled',
};

export default function FinishingChecklistPage() {
  const { token } = useAuth();
  const [items, setItems] = useState<FinishingChecklistItem[]>([]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.finishingChecklist.create(token, {
        title: form.title,
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Kenar yaldız, ebru ve kalite kontrol planları</p>
          </div>
              ...
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="category" value={form.category} onChange={(e) => update('category', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {CATEGORIES.map((c) => (
                      <option key={c} value={c}>{formatFinishingCategory(c)}</option>
                    ))}
                  </select>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="zone">Kanat / Bölüm</Label>
                  <Input id="zone" value={form.zone} onChange={(e) => update('zone', e.target.value)} placeholder="Örn: Ana Atölye" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="status" value={form.status} onChange={(e) => update('status', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatFinishingStatus(s)}</option>
                    ))}
                  </select>
                </div>
        ...
        {!loading && items.length > 0 && (
          <div className="space-y-3">
            {items.map((item) => (
              <Card key={item.id} className="archive-card">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {formatFinishingCategory(item.category)} · {item.zone || 'Genel'} · {formatDateTime(item.scheduledAt)}
                      </p>
                    </div>
                  </div>
                  <Badge variant="secondary">{formatFinishingStatus(item.status)}</Badge>
                </CardContent>
              </Card>
            ))}
          </div>
