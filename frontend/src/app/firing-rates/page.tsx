...
import {
  formatCurrency,
  formatFiringRateStatus,
  formatFiringCategory,
} from '@/lib/utils';
...
export default function FiringRatesPage() {
  const { token } = useAuth();
  const [tiers, setTiers] = useState<FiringRate[]>([]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.firingRates.create(token, {
        title: form.title,
            ...
            <CardContent>
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  >
                    {CATEGORIES.map((c) => (
                      <option key={c} value={c}>{formatFiringCategory(c)}</option>
                    ))}
                  </select>
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2 sm:col-span-2">
                  ...
                  >
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatFiringRateStatus(s)}</option>
                    ))}
                  </select>
                </div>
                  ...
                  <tbody>
                    {tiers.map((tier) => (
                      <tr key={tier.id} className="border-b border-muted hover:bg-muted/30">
                        ...
                        <td className="p-3">{formatFiringCategory(tier.rateCategory)}</td>
                        ...
                        <td className="p-3">
                          <Badge variant="secondary">{formatFiringRateStatus(tier.status)}</Badge>
                        </td>
                      </tr>
                    ))}
                  </tbody>
