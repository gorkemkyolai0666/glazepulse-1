...
interface PotteryStudioProfile {
  name: string;
  phone?: string;
  address?: string;
  city?: string;
...
export default function SettingsPage() {
  const { token } = useAuth();
  const [venue, setVenue] = useState<PotteryStudioProfile | null>(null);
  ...
  const load = () => {
    ...
    api.potteryStudio
      .get(token)
      .then((data) => setVenue(data as PotteryStudioProfile))
      .catch(() => setError(true))
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSuccess(false);
    try {
      await api.potteryStudio.update(token, venue as unknown as Record<string, unknown>);
      setSuccess(true);
    } catch {
      setError(true);
    } finally {
          ...
          <Card className="kiln-card">
            ...
            <CardContent>
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="totalKilns">Toplam Oda Sayısı</Label>
                  <Input
                    id="totalKilns"
                    type="number"
                    value={venue.totalKilns}
                    onChange={(e) =>
                      setVenue({ ...venue, totalKilns: parseInt(e.target.value, 10) || 0 })
                    }
                  />
                </div>
                <Button type="submit" disabled={saving} className="kiln-btn">
