...
export default function KilnMaintenancePage() {
  const { token } = useAuth();
  const [items, setItems] = useState<KilnMaintenanceItem[]>([]);
  ...
  const load = () => {
    ...
    setLoading(true);
    setError(false);
    Promise.all([api.kilnMaintenance.list(token), api.kilns.list(token)])
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.kilnMaintenance.create(token, {
        kilnId: form.kilnId,
        title: form.title,
              ...
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="priority" value={form.priority} onChange={(e) => update('priority', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {PRIORITIES.map((p) => (
                      <option key={p} value={p}>{formatKilnMaintenancePriority(p)}</option>
                    ))}
                  </select>
                </div>
                <div className="space-y-2">
                  ...
                  <select id="status" value={form.status} onChange={(e) => update('status', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatKilnMaintenanceStatus(s)}</option>
                    ))}
                  </select>
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.kiln?.name || 'Fırın belirtilmemiş'} · {item.kiln?.zone} · {formatKilnMaintenancePriority(item.priority)} · {formatDateTime(item.reportedAt)}
                      </p>
                    </div>
                  </div>
                  <Badge variant="secondary">{formatKilnMaintenanceStatus(item.status)}</Badge>
                </CardContent>
              </Card>
            ))}
          </div>
