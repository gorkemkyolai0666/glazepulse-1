...
import { formatMaterialOrderStatus, formatCurrency } from '@/lib/utils';
...
export default function MaterialOrdersPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.materialOrders.create(token, {
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="font-display text-3xl text-primary">Malzeme Siparişleri</h1>
            ...
            <CardContent>
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2 sm:col-span-2">
                  <Label htmlFor="materialType">Kil Kategorisi</Label>
                  <Input id="materialType" value={form.materialType} onChange={(e) => update('materialType', e.target.value)} required placeholder="Örn: İpek Saten Set" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="status" value={form.status} onChange={(e) => update('status', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatMaterialOrderStatus(s)}</option>
                    ))}
                  </select>
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.materialType} · {item.supplierName || 'Tedarikçi belirtilmemiş'} · {formatCurrency(item.price)}
                      </p>
                    </div>
                  </div>
                  <Badge variant="secondary">{formatMaterialOrderStatus(item.status)}</Badge>
                </CardContent>
              </Card>
            ))}
          </div>
