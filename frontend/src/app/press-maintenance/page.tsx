...
interface PressMaintenanceItem {
  id: string;
  title: string;
  description?: string;
  priority: string;
...
export default function PressMaintenancePage() {
  const { token } = useAuth();
  const [items, setItems] = useState<PressMaintenanceItem[]>([]);
  ...
  const load = () => {
    ...
    setLoading(true);
    setError(false);
    Promise.all([api.pressMaintenance.list(token), api.presses.list(token)])
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.pressMaintenance.create(token, {
            ...
            <CardContent>
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2 sm:col-span-2">
                  <Label htmlFor="pressId">Terzi Cilt Atölyesisi</Label>
                  <select
                    id="pressId"
                    value={form.pressId}
                ...
                <div className="space-y-2 sm:col-span-2">
                  ...
                  <Input id="title" value={form.title} onChange={(e) => update('title', e.target.value)} required placeholder="Örn: Nipping press menteşe ayarı" />
                </div>
                <div className="space-y-2 sm:col-span-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="priority" value={form.priority} onChange={(e) => update('priority', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {PRIORITIES.map((p) => (
                      <option key={p} value={p}>{formatPressMaintenancePriority(p)}</option>
                    ))}
                  </select>
                </div>
                <div className="space-y-2">
                  ...
                  <select id="status" value={form.status} onChange={(e) => update('status', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatPressMaintenanceStatus(s)}</option>
                    ))}
                  </select>
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.press?.name || 'Pres belirtilmemiş'} · {item.press?.zone} · {formatPressMaintenancePriority(item.priority)} · {formatDateTime(item.reportedAt)}
                      </p>
                    </div>
                  </div>
                  <Badge variant="secondary">{formatPressMaintenanceStatus(item.status)}</Badge>
                </CardContent>
              </Card>
            ))}
          </div>
